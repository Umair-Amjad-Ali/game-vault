      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          ...
          <p className="text-xs sm:text-sm text-gray-700 mt-2 leading-relaxed font-semibold max-w-2xl mx-auto">
            Discover how USA Gaming Distributor has helped store owners and
            agents establish their gaming portals and maximize weekly credits.
          </p>
        </motion.div>
          ...
          <div className="relative w-full h-[380px] flex justify-center items-center perspective-1000 overflow-visible">
            ...
            >
              {getVisibleTestimonials().map(({ item, position }) => {
                ...
                // Inverted Position offsets
                let transformX = 0;
                if (isPrev) transformX = 220; // Shift right (Swapped)
                if (isNext) transformX = -220; // Shift left (Swapped)

                return (
                  <motion.div
                    key={item.id}
                    custom={direction}
                    // Inverted initial and exit animations
                    initial={{
                      opacity: 0,
                      scale: 0.8,
                      x: direction > 0 ? -350 : 350, // Swapped
                      filter: "blur(4px)",
                    }}
                    ...
                    className={`absolute overflow-hidden cursor-pointer ${
                      isActive
                        ? "w-[280px] md:w-[320px]"
                        : "w-[240px] md:w-[280px] hidden md:block"
                  ...
                  >
                    {/* Card Container */}
                    <div
                      className={`w-full h-full min-h-[350px] p-6 transition-all duration-500 flex flex-col ${
                        isActive
                          ? "bg-linear-to-br from-pink-500 via-pink-600 to-slate-900 text-white"
                      ...
                      <div className="relative z-10 flex flex-col h-full text-left grow justify-between">
                        <div>
                          ...
                          <div className="flex items-center gap-3 mb-4">
                            <div
                              className={`w-11 h-11 rounded-full border-2 overflow-hidden shadow-xs shrink-0 ${
                                isActive ? "border-white/30" : "border-pink-100"
                              }`}
                            >
                              <img
                                src={item.image}
                            ...
                            <div>
                              ...
                              <p
                                className={`text-[10.5px] font-extrabold tracking-wide ${
                                  isActive ? "text-pink-200" : "text-pink-600"
                                }`}
                              >
                                {item.title}
                              </p>
                          ...
                          <div className="mb-4">
                            <span
                              className={`text-[8px] uppercase font-black tracking-widest leading-none ${
                          ...
                          {/* Quote Text */}
                          <p
                            className={`text-[11px] leading-relaxed font-semibold italic ${
                        ...
                        <div
                          className={`mt-5 pt-3 border-t ${
                            isActive ? "border-white/10" : "border-pink-50"
                          }`}
                        >
                          <Link
                            href="/become-a-distributor"
                            className={`inline-flex items-center gap-1 text-[10.5px] font-black uppercase tracking-wider transition-all hover:gap-2 cursor-pointer ${
